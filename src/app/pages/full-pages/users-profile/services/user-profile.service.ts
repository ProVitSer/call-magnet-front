...
import { BaseUsersResponse, ChangePasswordData, ChangePasswordResponse, CheckVerificationCodeResponse, ForogtPasswordData, ResetPasswordData, UpdateClientInfoData, UpdateClientInfoResponse, UserInfoResponse } from "../models/user-info";
...
export class UserProfileService {
    private readonly userProfileUrl = `${environment.API_GATEWAY_URL}/user`;
  ...
  constructor(    
    ...
    public getUserInfo(userId: number): Observable<UserInfoResponse> {
      return this.http
        .get<UserInfoResponse>(`${this.userProfileUrl}/${userId}`)
        .pipe(catchError(this.errorHandler));
    }

    public updateUserInfo(data: UpdateClientInfoData): Observable<UpdateClientInfoResponse> {
      return this.http
        .put<UpdateClientInfoResponse>(`${this.userProfileUrl}`, data)
        .pipe(catchError(this.errorHandler));
    }
    ...
    public changePassword(data: ChangePasswordData): Observable<ChangePasswordResponse> {
      return this.http
        .post<ChangePasswordResponse>(`${this.userProfileUrl}/change-password`, data)
        .pipe(catchError(this.errorHandler));
    }
    
      public resetPassword(data: ResetPasswordData): Observable<BaseUsersResponse> {
        return this.http
          .post<BaseUsersResponse>(`${this.userProfileUrl}/reset-password`, data)
          .pipe(catchError(this.errorHandler));
      }
    
      public forogtPassword(data: ForogtPasswordData): Observable<BaseUsersResponse> {
        return this.http
          .post<BaseUsersResponse>(`${this.userProfileUrl}/forgot-password`, data)
          .pipe(catchError(this.errorHandler));
      }
